...
import { teaOpexUnitRedux, update_tea_opex_cost_waste } from "../../redux/features/tea/TEAOpexSlice";
...
function TEAOpexInputWastePage() {
  ...
  const columnHeaders = ["Waste", "Mass Flow", "Unit", "Cost (1kg)", "", ""];
  ...
  useEffect(()=>{
    if(!AllowedPages.opexInputWaste){
      navigate(-1);
    }
  ...
  const handleOpexInputWasteSave = () => {
    ...
    const nextUrl = getNavURL("/tea/opex-results-by-stream");
    reduxDispatch(update_tea_opex_cost_waste(finalArr))        
    reduxDispatch(update_allowed_tea_pages({ opexResultsByStream: true, opexRevenueProduct: true }));
    navigate(nextUrl);
  }
  ...
  const exportFileName = `TEA-Opex-Input-Waste-Export (${new Date().toISOString()})`
    ...
    return(
      <div>
        <AppPageLayout sideMenu={<TEASideMenu/>}>
          ...
          onSave={handleOpexInputWasteSave}  
...
export default TEAOpexInputWastePage;