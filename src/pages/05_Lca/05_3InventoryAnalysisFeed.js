...
import { update_lca_input_processes_feed } from "../../redux/features/lca/LCAInputSlice";
...
const initRowObj =   {
  ...
  columns: [
    ...
    },
    {
      colId: "01",
      colType: "dropdown",
      colMenuItems: ["Raw material", "Ancillary material"/*, "Product"*/],
      colValue: "Raw material"
    },
    {
      colId: "02",
      colType: "checkbox",
...
function LCAInventoryAnalysisFeed() {
  ...
  },[AllowedPages, navigate, reduxDispatch, userToken, SelectedProjectId])
  ...
  const handleInventoryAnalysisFeedSave = () => {
    let finalArr = []

    tableData.forEach(row => {
      let newRowItem = {comp_id: null, process_name: null, uuid: null, description: "", materialCategory: null, isSelected: false, amounts: 0, unit: ""}

      newRowItem["comp_id"]          = row.columns[0].colValue;
      newRowItem["materialCategory"] = row.columns[1].colValue;
      newRowItem["isSelected"]       = row.columns[2].colValue;
      newRowItem["process_name"]     = row.columns[3].colValue.value;
      newRowItem["uuid"]             = row.columns[3].colValue.uuid;
      newRowItem["description"]      = row.columns[3].colValue.description;
    ...
    if(!shouldSkip){
      const nextUrl = getNavURL("/lca/input-inventory-analysis-utility");
      reduxDispatch(update_allowed_lca_pages({inputInventoryAnalysisUtility: true}))
      reduxDispatch(update_lca_input_processes_feed(finalArr))
      navigate(nextUrl)
    };
  }
  ...
  const exportFileName = `LCA-Input-Inventory-Analysis-Feed-Export (${new Date().toISOString()})`
...
export default LCAInventoryAnalysisFeed;