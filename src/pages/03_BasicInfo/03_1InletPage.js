...
import { update_basic_info_inlet_stream_class } from "../../redux/features/basicInfo/basicInfoStreamClassSlice";
...
function BIInletPage() {
  ...
  useEffect(()=>{
    let serverData = allowServerComm? BIInletData : BIInOuletSample[0]["u_inlet_strm"]
  ...
  const StreamClassDropdowns = () => {
    ...
    const renderedDropdowns = streamClassList.map(thisClass => {
      ...
      return <td key={thisClass._id} className='has-text-centered'><SelectDropdown options={[null, "Feed"]} value={thisClass.stream_class} onChange={handleDropChange}/></td>
    })
  ...
  const onStreamClassListSave = () => {
    ...
    const nextUrl = getNavURL("/basic-info/aspen-info-streams-outlet");
    reduxDispatch(update_basic_info_inlet_stream_class(streamClassList))
    reduxDispatch(update_allowed_basic_info_pages({outlet: true}))
